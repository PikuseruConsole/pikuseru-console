...
use network::PikuseruConsoleState;
...
pub struct PikuseruConsole {
    pub engine: Arc<Mutex<pikuseru::core::Pikuseru>>,
    pub fps: fps::FpsCounter,
}

impl PikuseruConsole {
    pub fn new(engine: pikuseru::core::Pikuseru, session: SessionDescriptor, max_prediction: usize) -> (Self, PikuseruConsoleState) {
    ...
    fn generate_save_state(&mut self) -> PikuseruConsoleState {
        ...
        PikuseruConsoleState {
            previous_buttons,
        }
    }

    pub fn load_save_state(&mut self, state: PikuseruConsoleState) {
        let engine = self.engine.lock().unwrap();
        let contexts = &mut engine.contexts.lock().unwrap();

        let PikuseruConsoleState {
            previous_buttons,
        } = state;
...
impl Console for PikuseruConsole {
    fn frames_per_second(&mut self) -> usize {
        let engine = self.engine.lock().unwrap();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  //  env::set_var("RUST_BACKTRACE", "1");
    ...
    let mut session: Option<P2PSession<PikuseruConsole>> = None;
    ...
    event_loop.run(move |event, _, control_flow| {       
        ...
        if input.update(&event) {
            ...
            if input.key_pressed(VirtualKeyCode::F1) {
                ...
                if let Some(console) = &mut framework.gui.pikuseru_console {
...
const DEFAULT_WINDOW_RESOLUTION: pikuseru::core::resolution::Resolution = pikuseru::core::resolution::Resolution::High;
...
fn init_pixels(window: &Window) -> Pixels {
    ...
    Pixels::new(pikuseru::core::DEFAULT_MODE_WIDTH as u32, pikuseru::core::DEFAULT_MODE_HEIGHT as u32, surface_texture).unwrap()
}