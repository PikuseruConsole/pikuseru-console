...
use crate::network::PikuseruConsoleState;

pub struct Gui {
    pub window_open: bool,
    pub game_file: Option<PathBuf>,

    pub pikuseru_console: Option<PikuseruConsole>,
    pub initial_state: Option<PikuseruConsoleState>,
...
impl Gui {
    fn ui(
        ...
        pixels: &mut Pixels,
        window: &Window,
        session: &mut Option<P2PSession<PikuseruConsole>>,
        ctx: &Context,
            ...
            .show(ctx, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        if ui.button("Select Game").clicked() {
                            self.game_file = FileDialog::new()
                                .add_filter("pik (.pik), p8 (.p8)", &["pik", "p8"])
                                .pick_file();
                        };
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        ...
                        .clicked()
                    {
                        let console = self.pikuseru_console.as_mut().unwrap();
    ...
    fn init_with_console(
        &mut self,
        rom: pikuseru::core::Pikuseru,
        pixels: &mut Pixels,
        window: &Window,
        session_descriptor: SessionDescriptor,
        width: u32,
        height: u32
    ) -> P2PSession<PikuseruConsole> {       
        ...
        let (mut console, reset) = PikuseruConsole::new(rom, session_descriptor, max_prediction);

        console.sync_audio();
        console.sync_mouse(window);

        self.pikuseru_console = Some(console);
    ...
    pub(crate) fn try_launch_game(
        ...
        pixels: &mut Pixels,
        window: &Window,
    ) -> Option<P2PSession<PikuseruConsole>> {
        let path = self.game_file.as_ref().unwrap();
        ...
        let mut rom = pikuseru::core::Pikuseru::new();
